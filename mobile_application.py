class Mobile:
    ...
    def uninstall(self):
        self.storage_details()
        name = input("Enter name of application to uninstall: ")
        print("Category of applications :")
        k = tuple(self.applications.keys())
        for i, j in enumerate(k):
            print(f"{i} : {j}")
        op = int(input("Select the category of application (like 0,1,2..): "))
        if name in self.applications[k[op]]:
            st = self.applications[k[op]][name]
            del self.applications[k[op]][name]
            self.Storage_used -= st
            print(f"{name} application uninstalled")
            print(self.applications)
            self.storage_details()
        else:
            print(f"{name} application not found")
